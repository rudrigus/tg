...
\item Selecionar imagens de curto circuito (brilhoMaximo=30\%)
...
Esta primeira etapa é bastante simples, pois basta somar os valores de intensidade dos \textit{pixels} e compará-los com um valor numérico pré definido. No Matlab, isto é feito com uma soma da matriz de imagem carregada que é então comparada com a variável $brilhoMaximo$. Essa variável teve inicialmente seu valor definido como 30\% do máximo possível para uma imagem. No caso de uma imagem de dimensões 296x264, esse valor é $brilhoMaximo = 3985344$.

\begin{figure}[h!]
\centering
\figuraresult{Img}{2420}{2439}
\caption{Imagens 2420 a 2440.}
\label{fig:desenv-imagens-selecao}
\end{figure}


\begin{figure}[h!]
\centering
\subfigure[\label{fig:desenv-selecao-imagens-.3}]{
	\centering
	\includegraphics[width=.7\textwidth]{./Figuras/desenv-grafico-selecao-03.jpg}}
\subfigure[\label{fig:desenv-selecao-imagens-.2}]{
	\centering
	\includegraphics[width=.7\textwidth]{./Figuras/desenv-grafico-selecao-02.jpg}}
\caption{(a) Imagens selecionadas com fator de 30\%. (b) Imagens selecionadas com fator de 20\%.}
\label{fig:desenv-selecao-imagens}
\end{figure}

Para o teste desta etapa são utilizadas as imagens de número 2420 a 2439 de uma filmagem não sincronizada denominada ''Resultados 2 DVd Guilherme'', a 1000 FPS. Essas imagens estão em sequência na figura \ref{fig:desenv-imagens-selecao}. A figura \ref{fig:desenv-selecao-imagens} Mostra quais imagens foram selecionadas para se fazer o processamento. Como pose-se observar, valor de 20\% teve melhor seletividade nesta coleção.


As figuras \ref {fig:desenv-imagens-selecao} e \ref{fig:desenv-selecao-imagens} demonstram que o algoritmo seleciona quase todas as imagens da série quando o fator de brilho é 30\% e seleciona apenas as que realmente se aproximam de um curto-circuito com zum fator de brilho de 20\%. Esse valor deve ser calibrado quando se muda o \textit{setup} do processo.

Após a seleção, as imagens boas são armazenadas em um \textit{buffer} para que se siga o processamento. Nesta etapa também os valores calculados são repetidos quando a imagem não é selecionada. Ou seja enquanto não surgirem imagens propícias ao processamento, as medidas desejadas permanecem com seu valor anterior.
...
A média de imagens não é um filtro muito comumente utilizado, mas se provou útil neste trabalho. Observou-se em várias imagens objetos brilhosos que aparecem aleatoriamente, provavelmente respingos de solda e fagulhas. São ruídos muito grandes para serem retirados com filtros de janela. O interessante é que estes objetos não costumam estar presentes em mais de uma imagem. As características do processo favorecem o uso da média, pois as imagens as formas a serem identificadas mudam pouco de \textit{frame} a \textit{frame}.
...
A princípio, desejava-se calcular a distorção de perspectiva e fazer os ajustes necessários durante o processamento das imagens do processo de solda. A ideia seria utilizar os cantos do eletrodo de solda (dois formados entre o topo do eletrodo e o  da tocha e dois entre a base do eletrodo e a poça de soldagem) para identificar os pontos necessários para este cálculo. No entanto, alguns fatos foram observados:
...
\subsection{Retirada de \textit{scanlines}}
...
O sistema em FPGA necessitou que este filtro fosse executado antes do filtro de média de imagens pois  seriam necessárias muitas leituras em memória para se obter a janela 3x3. Dessa forma o filtro de gaussiana é aplicado logo em seguida à primeira binarização e o \textit{pixels} filtrados são armazenados em memória para a filtragem por média.

Como explicado na seção \ref{section:realce-imagens}, este é um filtro que utiliza uma janela de \textit{pixels} em uma imagem e uma máscara. A máscara escolhida para se reduzir o ruído foi a de gaussiana, e pode ser vista na tabela abaixo \ref{table:mascara-gaussiana}.

\begin{table}[h!]
\centering
\begin{tabular}{|c|c|c|}
\hline
0,0113 & 0,0838 & 0,0113 \\ \hline
0,0838 & 0,6193 & 0,0838 \\ \hline
0,0113 & 0,0838 & 0,0113 \\ \hline
\end{tabular}
\label{table:mascara-gaussiana}
\caption{Máscara de gaussiana.}
\end{table}

Para evitar se trabalhar com pontos flutuantes na FPGA, os valores da tabela \ref{table:mascara-gaussiana} foram multiplicados por $2^8$ para se fazer a multiplicação pela janela de \textit{pixels}. O resultado da multiplicação sofre um \textit{shift} para a direita para se realizar uma divisão do resultado por $2^8$. Os novos valores podem ser vistos na tabela \ref{table:mascara-gaussiana-fpga}.
...
03 & 15 & 03 \\ \hline
\end{tabular}
\label{table:mascara-gaussiana-fpga}
\caption{Máscara de gaussiana para FPGA (valores hexadecimais).}
\end{table}

A janela para o filtro é criada como descrito na seção \ref{section:tecnicas-filtro-fpga}. Ficou decidido, por questões de simplicidade e pela natureza das imagens usadas, que seriam descartadas as primeiras e últimas linhas e colunas das imagens. Dessa forma o \textit{pixel} processado está sempre uma linha e uma coluna atrás do novo \textit{pixel} recebido e o processamento termina quando o último é recebido.
...
\subsection{Cálculo de topo e base do arame} \label{section:topo-base}

Os dados de topo e base do arame foram obtidos com a mesma lógica criada para Matlab e de forma sequencial. Foi utilizado um processo que tem $coluna$, $linha$ e $in\_clock$ como lista de sensitividade. Os sinais $coluna$ e $linha$ foram utilizados para identificar a posição que o algoritmo está na imagem e o $in\_clock$ é o \textit{clock} de entrada da câmera e define o fim e o começo da transmissão de cada pixel.

Foram criados os mesmos vetores $somaHor$ e o $derivadaHor$ ambos do tipo $vetorHor$. Como citado em \ref{section:desenv-fpga-filtro_media_imagens} os elementos de $somaHor$ são calculados a cada pixel recebido. 

É nesta etapa onde a média de imagens é realmente aplicada. O vetor $somaHor$ é sempre acrescido do valor do \textit{pixel} recebido e subtraído do \textit{pixel} mais antigo armazenado na memória, recebido como entrada no sinal $q$.

Os valores de $derivadaHor$ apenas são calculados ao final da leitura de cada linha. Ao mesmo tempo são definidos os valores $posArameTopo$ e $posArameBase$ com simples testes também ao final de cada linha. A derivada no índice da linha finalizada, $derivadaHor(linha)$ é testada para se definir as medidas buscadas. Ao final da imagem estes valores já estão devidamente calculados.


\subsection{Bordas da poça de soldagem}\label{section:desenv-bordas-poca-fpga}

A lógica para este cálculo é semelhante à obtenção do topo e base do eletrodo na seção \ref{section:topo-base}, porém as bordas da poça são calculadas apenas ao se receber a última linha de uma imagem. Este processo ocorre dessa forma pois os elementos do vetor $somaVert$ só têm seus valores definidos na última linha. Consequentemente o vetor $derivadaVert$ só pode ser definido também na última linha. 
...
Para a obtenção dos pontos é feita a derivada \textit{pixel} a \textit{pixel} na direção horizontal, a cada linha. Quando essa derivada atinge um mínimo ou máximo local, o vetor $inicioArame0$ tem seu valor definido no índice daquela linha como a coluna onde ocorreu o mínimo ou o máximo.

Para que seja aplicada a média de imagens, é necessário armazenar cópias desse vetor para cada nova imagem, são atualizados em forma de média móvel. Como $quantidadeImagens=3$, são necessários mais três vetores. As bordas do eletrodo são definidas como a média dos três vetores entre os índices $posArameTopo$ e $posArameBase$.
...
\subsection{Simulações em FPGA} \label{section:desenv-simulacoes-fpga}
